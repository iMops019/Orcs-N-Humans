...
// resolveCircleCollisions) afterward - this only sets direction.
...
void updatePlayerAttack(engine::ecs::Registry& registry, PlayerState& player, float dt);
...
// invulnerability window. Also ticks that window down by dt.
void updateContactDamage(engine::ecs::Registry& registry, PlayerState& player, float dt);
